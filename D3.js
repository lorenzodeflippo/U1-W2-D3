...
while (i < starWarsCharacters.length) {
  crewMass += starWarsCharacters[i].mass
    ? parseInt(starWarsCharacters[i].mass)
    : 0;
  i++;
}
...
for (let i = characters.length - 1; i >= 0; i--) {
  if (femaleNames.includes(characters[i])) {
    characters.splice(i, 1);
  }
}
console.log("Length of 'characters' array before:", characters.length);
console.log("Updated 'characters' array:", characters.length);
console.log("Length of 'characters' array after:", characters.length);
/* --EXTRA-- ESERCIZIO 10
...
const randomIndex = getRandomInt(0, starWarsCharacters.length - 1);
const randomCharacter = starWarsCharacters[randomIndex];
const message =
  "Parliamo di un personaggio di Star Wars! Che ne pensate di ${randomCharacter.name}? Questo personaggio ha capelli ${randomCharacter.hair_color}, occhi ${randomCharacter.eye_color} ed era nato nel ${randomCharacter.birth_year}. Ti piacerebbe sapere altro?";
console.log(message);
